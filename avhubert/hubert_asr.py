...
def _unwrap_seq2seq_checkpoint(w2v_args, state):
    """Accept AV-HuBERT seq2seq checkpoints as encoder init checkpoints.

    Upstream fine-tuning expects a pretraining checkpoint whose model keys are
    unprefixed, e.g. ``feature_extractor_video...``. Some released/derived
    checkpoints are seq2seq checkpoints and store the same encoder under
    ``encoder.w2v_model.*`` plus decoder weights. For adaptation we only need
    the encoder, so strip that prefix and use the nested pretraining config.
    """
    inner_w2v_args = _cfg_get(w2v_args, "model", "w2v_args")
    model_name = _cfg_get(w2v_args, "model", "_name")
    if state is not None and model_name == "av_hubert_seq2seq" and inner_w2v_args is not None:
...
class HubertEncoder(FairseqEncoder):
    def __init__(self, cfg: AVHubertAsrConfig, tgt_dict=None):
        ...
        if cfg.w2v_args is None:
            ...
            w2v_args, state = _unwrap_seq2seq_checkpoint(w2v_args, state)
