...
def _main(cfg, output_file):
    ...
    # Keep the fine-tuned checkpoint's original encoder init path. Overriding
    # w2v_path with cfg.common_eval.path makes seq2seq checkpoints recurse into
    # themselves while building the encoder.
    arg_overrides = {}
    if cfg.override.data is not None:
        arg_overrides.setdefault("task", {})["data"] = cfg.override.data
    if cfg.override.label_dir is not None:
        arg_overrides.setdefault("task", {})["label_dir"] = cfg.override.label_dir
        spm_model = os.path.join(cfg.override.label_dir, "spm_unigram1000.model")
        if os.path.exists(spm_model):
            arg_overrides.setdefault("task", {})["tokenizer_bpe_model"] = spm_model
        pretrain_cfg_path = Path(__file__).resolve().parent / "conf" / "pretrain" / "large_vox_iter5.yaml"
        if os.path.exists(pretrain_cfg_path):
            w2v_args = OmegaConf.load(pretrain_cfg_path)
            OmegaConf.set_struct(w2v_args, False)
            w2v_args.common.user_dir = cfg.common.user_dir
            w2v_args.task.data = cfg.override.label_dir
            w2v_args.task.label_dir = cfg.override.label_dir
            w2v_args.task.labels = ["km"]
            w2v_args.task.label_rate = 25
            w2v_args.task.sample_rate = 25
            w2v_args.task.modalities = ["audio", "video"]
            w2v_args.task.fine_tuning = False
            w2v_args.model.label_rate = 25
            arg_overrides.setdefault("model", {})["w2v_args"] = w2v_args
    models, saved_cfg, task = checkpoint_utils.load_model_ensemble_and_task(
        [cfg.common_eval.path], arg_overrides=arg_overrides
    )
